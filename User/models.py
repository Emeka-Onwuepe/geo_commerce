...
class UserManager(BaseUserManager):
    def create_user(self,email,full_name='null',phone_number=None,cluster=None, password=None):
        if not email:
            raise ValueError('Users must have an email address')
        user = self.model(email=self.normalize_email(email),full_name=full_name,
                          phone_number=phone_number,cluster=cluster)
...
class User(AbstractBaseUser,PermissionsMixin):
    ...
    cluster = models.ForeignKey(Cluster, verbose_name="cluster", related_name="cluster", on_delete=models.SET_NULL,null=True)
    is_super_admin = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    is_admin = models.BooleanField(default=False)
    staff=models.BooleanField(default=False)
    
    # override the ManyToMany fields from PermissionsMixin to avoid reverse accessor name clashes
