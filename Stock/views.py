from django.http import HttpResponseRedirect
from django.shortcuts import render
from django.urls import reverse
from django.contrib.auth.decorators import login_required
from Branch.models import Cluster,  Product_Size
from Product.models import Product, Product_Type
from Stock.forms import ProductStockForm

from Stock.models import  Product_Stock

from django.db.models import Q

# Create your views here.

@login_required(login_url="user:loginView")
def stockView(request,stockId,clusterId,action,pgroup):
    mapper = {'product':[Product_Stock,ProductStockForm,Product_Size,Product],
            #   'suits':[Suit_Stock,SuitStockForm,Suit_Size,Suit],
            #   'top':[Tops_Stock,TopsStockForm,Tops_Size,Top],
            #   'foot_wear':[Foot_Wear_Stock,FootWearStockForm,Foot_Wear_Size,Foot_Wear],
    
              }
    cluster = cluster.objects.get(pk=clusterId)
    form = mapper[pgroup][1]()
    
    cluster_stock = []
    productType = Product_Type.objects.all()
    stocks = mapper[pgroup][0].objects.filter(cluster_id = clusterId)[:10]

     
    if stockId != 0:   
        stock_instance = mapper[pgroup][0].objects.get(id=stockId)
        form = mapper[pgroup][1](instance=stock_instance)
        
    
    if request.method == "POST" and action == "get":
        data = request.POST
        pgroup = data['pgroup']
        cluster_stock = mapper[pgroup][2].objects.filter(
                                                        # cluster_instance__cluster = cluster
                                                         Q(cluster_instance__product__product_type__id = int(data['product_type'])) &
                                                         Q(cluster_instance__cluster = cluster.id) &
                                                         Q(cluster_instance__product__brand__iexact = data['brand']) &
                                                         Q(cluster_instance__product__type__iexact = data['type']) 
                                                        )
        
        # print(cluster_stock)
        
    if request.method == "POST" and action == "add":
        form = mapper[pgroup][1](data= request.POST,files=request.FILES)
        # form.data['cluster'] = cluster
        data = request.POST
        # pgroup = data['pgroup']
        if form.is_valid():
            form.save()
            # product.sizes.all()
            product  = mapper[pgroup][3].objects.get(pk=data['product'])
            cluster_stock = mapper[pgroup][2].objects.filter(
                                                        # cluster_instance__cluster = cluster
                                                         Q(cluster_instance__product__product_type__id = product.product_type.id) &
                                                         Q(cluster_instance__cluster = cluster.id) &
                                                         Q(cluster_instance__product__brand__iexact = product.brand) &
                                                         Q(cluster_instance__product__type__iexact = product.type) 
                                                        )

            # return HttpResponseRedirect(reverse('stock:stockView',
            # kwargs={"action":"view","stockId":0,'pgroup':pgroup,'clusterId':clusterId}))
        else:
            return render(request,"stock/stock.html",
                  {"form":form,"stockId":0,"action":"add",
                   'clusterId':clusterId,'pgroup':pgroup})  
            
    if action == "edit" and request.method == "POST":
            
        form = mapper[pgroup][1](data= request.POST,files=request.FILES,instance=stock_instance)
        if form.is_valid():
            form.save()

            return HttpResponseRedirect(reverse('stock:stockView',
                        kwargs={"action":"view","stockId":0,'pgroup':pgroup,'clusterId':clusterId}))
        else:
            return render(request,"stock/stock.html",
                  {"form":form,'clusterId':clusterId,
                   "stockId":stock_instance.id,
                   "action":"edit",'pgroup':pgroup})
    elif action == "edit" and request.method == "GET":
        return render(request,"stock/stock.html",
                  {"stock_instance":stock_instance,
                   'stocks':stocks,
                   "stockId":stock_instance.id,
                   'clusterId':clusterId,
                   "action":"edit",
                   'form':form,
                    'pgroup':pgroup})
    
    if action == "delete":
        stock_instance.delete()
        return HttpResponseRedirect(reverse('stock:stockView',
            kwargs={"action":"view","stockId":0,'pgroup':pgroup,'clusterId':clusterId}))
    # cluster_stock = None
     
    # cluster_stock = mapper[pgroup][2].objects.filter(cluster_instance__cluster = cluster)

    return render(request,"stock/stock.html",
                  {"stockId":0, 
                   'stocks':stocks,
                   "action":"add",
                   'cluster_stock':cluster_stock,
                   'clusterId':clusterId,
                    'cluster':cluster.name,
                   'cluster':cluster,
                    'form':form,
                   'pgroup':pgroup,
                   'productType':productType})
    

@login_required(login_url="user:loginView")
def getStockView(request):
    
    mapper = {'product':[Product_Size],
              }
    
    #  'cluster': [''], 'pgroup': ['suits'], 'gender': [''], 'age_group': [''], 'product_type': ['sss'], 'brand': ['sss'], 'type': ['ssss'], 'color': ['sss']
    stocks = None
    searched = False
    all = False
    data = []
    if request.method == "POST":
        searched = True
        data = request.POST
        model = data['pgroup']
        if data['cluster'] != '0':
            
            
            stocks = mapper[model][0].objects.filter(
                                             Q(cluster_instance__product__product_type__id = int(data['product_type'])) &
                                              Q(cluster_instance__cluster = data['cluster']) &
                                              Q(cluster_instance__product__brand__iexact = data['brand']) &
                                              Q(cluster_instance__product__type__iexact = data['type']) 
                                              )
        else:
            stocks = mapper[model][0].objects.filter(
                                              Q(cluster_instance__product__product_type__id = int(data['product_type'])) &
                                              Q(cluster_instance__product__brand__iexact = data['brand']) &
                                              Q(cluster_instance__product__type__iexact = data['type']) 
                                              ).select_related('cluster_instance')
        
            
            data_ = {}
            for stock in stocks:
                key = f'{stock.cluster_instance.product_id}_{stock.size_id}'
                if data_.get(key):
                    data_[key][1] = data_[key][1] + stock.current_qty
                    data_[key][2] = data_[key][2] + stock.returned_qty
                    data_[key][3] = data_[key][3] + stock.bad_qty
                else:
                    data_[key] = [stock,stock.current_qty,stock.returned_qty,stock.bad_qty]

            all = True
            stocks = []
            data = data_.values()
    productType = Product_Type.objects.all()
    # print(productType)
            # print(list(data)[0][0].cluster_instance.product.get_details())
    return render(request,"stock/getstock.html",
                  {
                   'stocks':stocks,
                   'searched':searched,
                   'all':all,
                   'all_':data,
                   'productType':productType
                  })
    