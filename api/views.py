...
class Get_cluster(generics.GenericAPIView):
...
class processSalesView(generics.GenericAPIView):
    ...
    #    data = json.loads(request.body.decode("utf-8"))
        ...
        cluster = cluster.objects.get(pk=int(data['cluster']))
        try:
        ...
        if data['payment_method'] == "credit":
            credit_or_sales = Credit_Sale.objects.create(cluster=cluster,
            ...
            credit_or_sales = Sales.objects.create(cluster=cluster,
        ...
        for product in data['data']:
            try:
                ...
                size_instance = Size.objects.get(size=size,gender=product['gender'],
                                        age_group = product['age_group'],
                                        product_type_id = product['product_type_id'] )
                cluster = cluster.objects.get(name__iexact = data['cluster'])
                
                check =  mapper[p_type.p_group][1].objects.filter(cluster = cluster,
                ...
                if check:
                    continue
                mapper[p_type.p_group][1].objects.create(cluster = cluster,
                                                          product = product_instance,
                                                          size_instance = size_instance,
                                                          qty = qty
                                                          )
