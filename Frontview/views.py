...
from geo_commerce.variables import official_email,rootUrl,admin_staff
...
#               'foot_wear':[Foot_Wear],
#               }
mapper = {'product':[Product,Cluster_Product,Product_Size],
            #   'suits':[Suit,cluster_Suit,Suit_Size],
            #   'top':[Top,cluster_Tops,Tops_Size],
            #   'foot_wear':[Foot_Wear,cluster_Foot_Wear,Foot_Wear_Size],
    ...
    for pgroup in mapper.values():
        ...
        if products:
            ...
            for product in products:
                filtered_sizes = []
                cluster_product = pgroup[1].objects.get(cluster__name='Main',
                                                            product = product)
                for size in product.sizes.all():
                    qty =  pgroup[2].objects.get(cluster_instance=cluster_product,
...
def categoryView(request,cat,brand,p_type):
    ...
    for product in products:
            filtered_sizes = []
            cluster_product = mapper[pgroup][1].objects.get(cluster__name='Main',
                                                            product = product)
            for size in product.sizes.all():
                qty =  mapper[pgroup][2].objects.get(cluster_instance=cluster_product,
                                                        size = size).current_qty
                if qty <=0:
                    filtered_sizes.append(size)
            sizes.append(filtered_sizes)
...
def processPaymentView(request):
       if request.method == "POST":
           ...
           if data['action'] == "create":
                ...
                cluster = cluster.objects.get(cluster__is_super_admin = True)
                sales = Sales.objects.create(cluster=cluster,
