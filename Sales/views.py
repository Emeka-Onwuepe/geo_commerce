...
def salesProductView(request,productTypeId,pgroup,brand='default'):
    json_data = {'products':[]}
    mapper = {'product':[Product,ProductSerializer,Cluster_Product,Product_Size],
            #   'suits':[Suit,SuitSerializer,cluster_Suit,Suit_Size],
            #   'top':[Top,TopSerializer,cluster_Tops,Tops_Size],
            #   'foot_wear':[Foot_Wear,FootWearSerializer,cluster_Foot_Wear,Foot_Wear_Size],
    ...
    if productTypeId != 0 and brand != 'default':
        ...
        for product in products:
            filtered_sizes = []
            cluster_product = mapper[pgroup][2].objects.get(cluster=request.user.cluster,
                                                           product = product)
            for size in product.sizes.all():
                qty =  mapper[pgroup][3].objects.get(cluster_instance=cluster_product,
...
def processSalesView(request):
   ...
   if request.method == "POST":
       ...
       if data['payment_method'] == "credit":
           credit_or_sales = Credit_Sale.objects.create(cluster=request.user.cluster,
           ...
           credit_or_sales = Sales.objects.create(cluster=request.user.cluster,
...
@login_required(login_url="user:loginView")     
def salesAnalysisView(request,date,clusterID):
    
    selected_cluster = request.user.cluster
    
    if clusterID != 0 and request.user.is_admin:
    ...
    products_data = []
    online_sales = []
    store = Items.objects.filter(items__date=date,items__channel='store',items__cluster=selected_cluster)
    transfer_total = Items.objects.filter(items__date=date,
                                       items__payment_method='transfer',
                                       items__cluster=selected_cluster).aggregate(total=Sum("total_price"))
    cash_total = Items.objects.filter(items__date=date,
                                items__cluster=selected_cluster,
                                       items__payment_method='cash').aggregate(total=Sum("total_price"))
    credit =  Items.objects.filter(credit_sale_Items__date =date,credit_sale_Items__cluster=selected_cluster)
...
@login_required(login_url="user:loginView")
def rangeSalesView(request,start_date,end_date,clusterID):

    selected_cluster = request.user.cluster.id
    
    if clusterID != 0 and request.user.is_admin:
        selected_cluster = clusterID
    
    cluster_instance = Cluster.objects.get(pk=selected_cluster)
    
    products_data = []
    online_sales = []
    store = Items.objects.filter(items__date__range=[start_date,end_date],
                                 items__channel='store',items__cluster=selected_cluster)
    credit =  Items.objects.filter(credit_sale_Items__date__range=[start_date,end_date],
                                   credit_sale_Items__cluster=selected_cluster)
    ...
    if request.user.is_admin:
        online_sales = Items.objects.filter(items__date__range=[start_date,end_date],
                                  items__cluster=selected_cluster, items__channel='web', items__paid=True)
    
        online = {
            "summary": online_sales.values('items__date','p_group').annotate(
                        total_amount=Sum("total_price"),total_qty =Sum("qty")),
            "total_amount":online_sales.aggregate(total=Sum("total_price"),logistics=Sum("items__logistics")),
        }
